...
public class ParkingHistoryComplexRepositoryImpl implements ParkingHistoryComplexRepository {
    ...
    @Override
    public List<ParkingHistoryEntity> findAll(String userNameOwner, DashboardModel requestModel) {
        ...
        if (!StringUtils.isEmpty(requestModel.getDateFrom()) && !StringUtils.isEmpty(requestModel.getDateTo())) {
            criteria.and("check_out_date").gte(requestModel.getDateFrom()).lte(requestModel.getDateTo());
        } else if (!StringUtils.isEmpty(requestModel.getDateTo())) {
            criteria.and("check_out_date").lte(requestModel.getDateTo());
        } else if (!StringUtils.isEmpty(requestModel.getDateFrom())) {
            criteria.and("check_out_date").gte(requestModel.getDateFrom());
        }
        return mongoTemplate.find(Query.query(criteria), ParkingHistoryEntity.class);
    }
